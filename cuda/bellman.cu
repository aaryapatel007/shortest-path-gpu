...
int runBellmanFordOnGPU(vector<int> adj_list, vector<int> adj_offset_list, vector<int> weights, int numVertices, int block_size, string output_file, float &msecs_gpu) {
    ...
    // compute time taken to run Bellman-Ford algorithm on GPU
    fprintf(stderr, "Computing shortest path using Bellman-Ford algorithm on GPU ");

    dim3 dimBlock(block_size);
	dim3 dimGrid(ceil(numVertices / float(block_size)));

    // initialize the distance array
    initializeInputArray<<<dimGrid, dimBlock>>>(d_distances, INT_MAX, numVertices);
    initializeInputArray<<<dimGrid, dimBlock>>>(d_distances_i, INT_MAX, numVertices);

    cudaEventRecord(start_gpu, 0);

    // Bellman ford
    for (int n = 0; n < numVertices - 1; n++) {
        bellmanFordGPUKernel<<<dimGrid, dimBlock>>>(d_adj_list_offsets, d_adj_list, d_weights, d_distances, d_distances_i, numVertices);
        updateDistanceArray<<<dimGrid, dimBlock>>>(d_distances, d_distances_i, numVertices);
    }
...
int runBellmanFordOnGPUGridStride(vector<int> adj_list, vector<int> adj_offset_list, vector<int> weights, int numVertices, int block_size, string output_file, float &msecs_gpu_gs) {
    ...
    // compute time taken to run Bellman-Ford algorithm on GPU
    fprintf(stderr, "Computing shortest path using Bellman-Ford algorithm on GPU with grid stride loop ");
    ...
    // Bellman ford
    for (int n = 0; n < numVertices - 1; n++) {
        bellmanFordGPUGridStrideKernel<<<dimGrid, dimBlock>>>(d_adj_list_offsets, d_adj_list, d_weights, d_distances, d_distances_i, numVertices);
        updateDistanceArrayGridStride<<<dimGrid, dimBlock>>>(d_distances, d_distances_i, numVertices);
    }
